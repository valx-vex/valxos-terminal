...
import { discoverClaudeCompat } from "../../src/compat/claude"
...
test("native config overrides Claude fallback while preserving Claude provenance", async () => {
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(config.claude?.provenance.settings.map((item) => item.scope)).toEqual(["user", "project", "local"])
      },
    })
  } finally {
...
test("malformed Claude settings stay diagnostic-only", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const claudeDir = path.join(dir, ".claude")
      await fs.mkdir(claudeDir, { recursive: true })
      await Filesystem.write(path.join(claudeDir, "settings.json"), '{"mcpServers": ')
    },
  })
  ...
  expect(discovery.config.mcp).toBeUndefined()
  expect(discovery.state?.provenance.diagnostics).toEqual(
    expect.arrayContaining([
      expect.objectContaining({
        path: path.join(tmp.path, ".claude", "settings.json"),
      }),
    ]),
  )
})
