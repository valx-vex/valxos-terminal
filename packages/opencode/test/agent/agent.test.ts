...
test("discovers agents from .claude/agents", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      ...
      await Bun.write(
        ...
        `---
description: Claude-compatible research agent
mode: subagent
tools:
  bash: false
---

You are a careful Claude-style research agent.
`,
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(researcher?.description).toBe("Claude-compatible research agent")
      expect(researcher?.prompt).toContain("Claude-style research agent")
