...
describe("Instruction.system Claude compatibility", () => {
  test("loads project CLAUDE.md as a system instruction", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const system = await Instruction.system()
        expect(system).toEqual(
          expect.arrayContaining([
            expect.stringContaining(`Instructions from: ${path.join(tmp.path, "CLAUDE.md")}`),
          ]),
        )
      },
    })
