...
function liveRuntime(layer: Layer.Layer<LLM.Service>, provider = ProviderTest.fake()) {
  ...
  const claude = ClaudeCompat.layer.pipe(Layer.provide(Config.defaultLayer))
  const processor = SessionProcessorModule.SessionProcessor.layer.pipe(Layer.provide(claude))
  return ManagedRuntime.make(
