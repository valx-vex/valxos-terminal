...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    legion: async (_provider) => {
      try {
        const mod = await import("@valxos/legion")
        return {
          autoload: true,
          async getModel(_sdk: any, modelID: string, options?: Record<string, any>) {
            return mod.createLegionModel(modelID, options)
          },
          options: {},
          async discoverModels() {
            return mod.LEGION_MODELS as unknown as Record<string, Model>
          },
        }
      } catch {
    ...
    Effect.gen(function* () {
      ...
      const state = yield* InstanceState.make<State>(() =>
        Effect.gen(function* () {
          ...
          // Seed Legion provider (self-contained, not from models.dev)
          const legionID = ProviderID.make("legion")
          if (!database[legionID] && !disabled.has(legionID)) {
            database[legionID] = {
              id: legionID,
          ...
          // Discover Legion models
          if (discoveryLoaders[legionID] && providers[legionID] && isProviderAllowed(legionID)) {
            yield* Effect.promise(async () => {
              try {
                const discovered = await discoveryLoaders[legionID]()
                for (const [modelID, model] of Object.entries(discovered)) {
                  if (!providers[legionID].models[modelID]) {
                    providers[legionID].models[modelID] = model
                  }
                }
              } catch (e) {
                log.warn("state discovery error", { id: "legion", error: e })
              }
            })
          }
