...
export namespace Config {
  ...
  async function loadAgentFiles(
    ...
    options?: {
      onInvalid?: (input: { path: string; message: string }) => void | Promise<void>
    },
  ) {
    ...
    for (const item of items) {
      const md = await ConfigMarkdown.parse(item).catch(async (err) => {
        ...
        await options?.onInvalid?.({ path: item, message })
        return undefined
      })
      if (!md) continue
  ...
  function writable(info: Info) {
    const { plugin_origins, claude, ...next } = info
    return next
  }
    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          const mergeClaude = (source: string, next: { mcp?: Info["mcp"] }, state?: ClaudeCompatState) => {
            if (next.mcp && Object.keys(next.mcp).length) merge(source, { mcp: next.mcp }, "local")
            if (state) claude = state
          }

          const claudeCompat = yield* Effect.promise(() =>
            discoverClaudeCompat({
          ...
          mergeClaude("claude-compat", { mcp: claudeCompat.config.mcp as Info["mcp"] }, claudeCompat.state)
          result.agent = mergeDeep(
            result.agent ?? {},
            yield* Effect.promise(() =>
              loadAgentFiles(claudeCompat.agentFiles, ["/.claude/agent/", "/.claude/agents/"], {
                onInvalid: ({ path, message }) => {
                  claudeCompat.state?.provenance.diagnostics.push({ path, message })
                },
              }),
            ),
          )
