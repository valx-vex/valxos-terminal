...
import { ClaudeCompat } from "@/compat/claude"
...
export namespace SessionPrompt {
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const claude = yield* ClaudeCompat.Service

      const requestPermission = Effect.fn("SessionPrompt.requestPermission")(function* (input: {
        cwd: string
        sessionID: SessionID
        permission: string
        patterns: string[]
        metadata: Record<string, unknown>
        request: () => Effect.Effect<void, Permission.Error>
      }) {
        const decision = yield* claude.permissionRequest({
          cwd: input.cwd,
        ...
        if (decision === "deny") {
          return yield* Effect.fail(new Permission.RejectedError())
        }
        yield* input.request()
      })

      const userPromptText = (message: MessageV2.WithParts | undefined) =>
        (message?.parts ?? [])
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
      ...
      const handleSubtask = Effect.fn("SessionPrompt.handleSubtask")(function* (input: {
        ...
        let result: Awaited<ReturnType<typeof taskTool.execute>> | undefined = yield* Effect.gen(function* () {
        ...
        if (result) {
          const post = yield* Effect.exit(
            Effect.gen(function* () {
              yield* plugin.trigger("tool.execute.after", { tool: "task", sessionID, callID: part.id, args: taskArgs }, result)
              yield* claude.postToolUse({
                cwd: ctx.directory,
                sessionID,
                tool: TaskTool.id,
          ...
          if (Exit.isFailure(post)) {
            const cause = Cause.squash(post.cause)
            error = cause instanceof Error ? cause : new Error(String(cause))
            log.error("subtask post hook failed", { error, agent: task.agent, description: task.description })
            result = undefined
          }
        }
      ...
      const runLoop: (sessionID: SessionID) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.run")(
        function* (sessionID: SessionID) {
          ...
          let lastHookedUserID: MessageID | undefined
          ...
          while (true) {
            ...
            const outcome: "break" | "continue" = yield* Effect.onExit(
              Effect.gen(function* () {
                ...
                const currentUserMsg = msgs.findLast((m) => m.info.role === "user" && m.info.id === lastUser.id)
                const claudeSystem: string[] = []
                if (!sessionStarted) {
                  claudeSystem.push(
                ...
                if (lastHookedUserID !== lastUser.id) {
                  claudeSystem.push(
                    ...(yield* claude.userPromptSubmit({
                      cwd: ctx.directory,
                      sessionID,
                      prompt: userPromptText(currentUserMsg),
                    })),
                  )
                  lastHookedUserID = lastUser.id
                }
                ...
                const system = [...env, ...(skills ? [skills] : []), ...instructions, ...claudeSystem]
          ...
          yield* claude.stop({ cwd: ctx.directory, sessionID, reason: "loop-complete" }).pipe(Effect.ignore)
          yield* claude.sessionEnd({ cwd: ctx.directory, sessionID, reason: "loop-complete" }).pipe(Effect.ignore)
  ...
  const defaultLayer = Layer.unwrap(
    Effect.sync(() =>
      layer.pipe(
        ...
        Layer.provide(ClaudeCompat.defaultLayer),
