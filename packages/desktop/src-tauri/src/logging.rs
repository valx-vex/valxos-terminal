...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("valxos-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("valxos_lib=debug,valxos_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("valxos_lib=info,valxos_desktop=info,sidecar=info")
        }
    });
