...
const deepLinkEvent = "valxos:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__VALXOS__ ??= {}
  const pending = window.__VALXOS__.deepLinks ?? []
  window.__VALXOS__.deepLinks = [...pending, ...urls]
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__VALXOS__?.wsl) return undefined
    return commands.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__VALXOS__?.wsl) return result
  ...
  return {
      ...
      await Promise.resolve()
        .then(() => {
          const notification = new Notification(title, {
            body: description ?? "",
            icon: "https://valxos.dev/favicon-96x96-v3.png",
          })
    ...
    getWslEnabled: async () => {
      ...
      return window.__VALXOS__!.wsl ?? false
    },
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("valxos.global.dat").getItem("language")
